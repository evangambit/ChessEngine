...
void print_variation(Thinker *thinker, Position* pos, Move move) {
  std::pair<Evaluation, std::vector<Move>> variationPair = thinker->get_variation(pos, move);
...
void mymain(std::vector<Position>& positions, const std::string& mode, double timeLimitMs, Depth depth, uint64_t nodeLimit, bool makeQuiet) {
  if (mode == "printvec" || mode == "printvec-cpu") {
    ...
    for (auto pos : positions) {
      ...
      SearchResult<Color::WHITE> results = search(&gThinker, goCommand, nullptr, [positions](Position *position, VariationHead<Color::WHITE> results, size_t depth, double secs) {
        if (positions.size() == 1) {
      ...
      std::vector<VariationHead<Color::WHITE>> topVariations(gThinker.variations);
